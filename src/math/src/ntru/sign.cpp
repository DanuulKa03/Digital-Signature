...
namespace ntru {
    ...
    bool sign_strict(const std::vector<uint8_t>& msg, Signature& sig) {
        ...
        for (int tries = 0; tries < G_MAX_SIGN_ATT; ++tries) {
            ...
            long double p = R / G_MACC; if (p > 1.0L) p = 1.0L;
            if (!std::isfinite((double)p) || p != p) p = 0.0L;

            uint32_t r = rng(); long double U = ((long double)r) / 4294967296.0L;
            if (U > p) continue;

            long double bound = (long double)G_ETA * (long double)G_SIGMA * sqrtl(2.0L * (long double)G_N);
            if (sqrtl(xnorm2) > bound) continue;

            sig.x1 = std::move(x1); sig.x2 = std::move(x2); sig.e = std::move(eh.e_mod);
            return true;
        }
        return false;
    }
